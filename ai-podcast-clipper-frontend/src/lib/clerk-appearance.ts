export const studioClerkAppearance = {
  ...
  elements: {
    card: "bg-[var(--superficie)] border border-[var(--linha)] shadow-[0_0_40px_rgba(0,0,0,0.8)] rounded-2xl p-6 sm:p-8",
    headerTitle: "text-[var(--marfim)] font-semibold tracking-tight text-xl",
    headerSubtitle: "text-[var(--fumaca)] text-sm",
    socialButtonsBlockButton:
      "bg-[var(--tinta)] hover:bg-[var(--superficie-2)] border border-[var(--linha)] text-[var(--marfim)] transition-colors py-2.5 rounded-full cursor-pointer",
    ...
    formFieldInput:
      "bg-[var(--tinta)] border border-[var(--linha)] focus:border-[var(--ouro)] text-[var(--marfim)] placeholder:text-[var(--fumaca)] transition-colors py-2 px-3 rounded-lg",
